...
2. 将全景图转为四向视角并拼接保存。
3. 调用 OpenAI API 对拼接图进行主观感知评分（CQ/AQ/HQ/VQ），支持 4 个 MLLM 组。
...
### 13) ALE 特征曲线（叠加对比）
默认使用 4 组：`mllm_baseline`、`mllm_anchor_prompt`、`mllm_rubric_prompt`、`mllm_local_prompt`，并将曲线叠加在同一图中（不同组不同颜色）。图例会单独输出为一张单行图片。
```bash
