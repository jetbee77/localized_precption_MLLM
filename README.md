...
### 1) 安装依赖
```bash
pip install -r ./svi_pipeline/requirements.txt
```
...
### 3) 筛选已标注街景
```bash
python ./svi_pipeline/scripts/prepare_labeled_images.py \
...
### 4) 生成四向拼接街景
```bash
python ./svi_pipeline/scripts/pano_to_stitched.py \
  --config ./svi_pipeline/configs/default.yaml
```
...
### 5) MLLM 感知评分（4 组）
```bash
python ./svi_pipeline/scripts/run_mllm_groups.py \
  --config ./svi_pipeline/configs/default.yaml
```

### 6) 监督学习训练与评估
```bash
python ./svi_pipeline/scripts/train_supervised.py \
  --config ./svi_pipeline/configs/default.yaml
```
...
### 7) 监督学习训练样本规模对比
```bash
python ./svi_pipeline/scripts/train_supervised_sizes.py \
  --config ./svi_pipeline/configs/default.yaml
```
可选参数：
...
```bash
python ./svi_pipeline/scripts/plot_distribution_compare.py \
  --config ./svi_pipeline/configs/default.yaml \
  --mllm-group baseline
```
...
### 10) 误差-特征散点图
```bash
python ./svi_pipeline/scripts/plot_error_feature_scatter.py \
  --config ./svi_pipeline/configs/default.yaml \
  --mllm-group baseline
```
可选：指定特征列表  
...
### 11) MLLM 全量推广预测（基线模型）
```bash
python ./svi_pipeline/scripts/predict_mllm_all_svi.py \
  --config ./svi_pipeline/configs/default.yaml \
  --output ./outputs/mllm_baseline_full.xlsx \
  --sleep 0.2 \
  --resume
```
...
```bash
pip install shap
python ./svi_pipeline/scripts/shap_rf_analysis.py \
  --config ./svi_pipeline/configs/default.yaml \
  --sample 300
```
...
### 13) ALE 特征曲线（叠加对比）
```bash
python ./svi_pipeline/scripts/ale_mllm_features.py
```
快速试运行（CQ）：
```bash
python ./svi_pipeline/scripts/ale_mllm_features.py --targets CQ
```
输出：  
